...
(function ($) {
  ...
  var settings = {
    ...
    input: '<input type="text" maxLength="20" placeholder="검색어 입력...">',
    data: [],
    searchable: true,
    searchNoData: '<li style="color:#ddd">내역이 없습니다..</li>',
    init: noop,
    choice: noop,
    extendProps: []
  };
  ...
  Dropdown.prototype = {
    ...
    renderSelect: function (isUpdate, isCover, noRedraw) {
      var _this = this;
      var $el = _this.$el;
...
//          $el.find('input').focus();
//        	$.proxy(action.show, _this)
        	if($el.hasClass("active"))
        		action.hide.apply(_this,[window.event]);
        	else
        		action.show.apply(_this,[window.event]);
    ...
    update: function (data, isCover,noRedraw) {
      var _this = this;
      var _config = _this.config;
      var $el = _this.$el;
    ...
    if(processResult){
      ...
      _this.renderSelect(true, _isCover,noRedraw);
    },
    destroy: function () {
      this.unbindEvent();
