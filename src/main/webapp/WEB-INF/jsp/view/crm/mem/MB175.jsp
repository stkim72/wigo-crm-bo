...
<div class="mSort2">
	<ul>
		<li class="wAuto">
			...
			<div class="txt">
				<div class="mDate1">
					<input type="text" data-type="month" maxlength= "6" class="it center" title="정산기준월" id="zrtDt" name="zrtDt" data-enter="search10" data-button="Y" value="">
				</div>
			</div>
		</li>
		<li class="wAuto">
			<div class="tit w105">부담율</div>
			<div class="txt">
				<input type="text" class="it" id="rate" name="rate" value="" />
			</div>
		</li>	
	</ul>
<!-- 	                <div class="gRt"> -->
...
<div class="mBox1">
	<div class="gTitle1">
		<h3 class="mTitle1">월별SAP(가맹포인트보전)</h3>
		<div class="gRt">
		    <a href="#;" class="mBtn1 r5 lPrimary" data-click="calculateStatusSAPSynthesis" id="btnCalculateStatusSAPSynthesis"><span>SAP IF</span></a>
			<code:btnGroup name="pointUseList" hideDelete="true" hideInsert="true" hideSave="true" dispName="월별SAP(가맹포인트보전)" gridId="grdList9"/>
...
<!-- 			</button> -->
	...
	<div  id="divGrid9" style="height:${rightBoxHeight4}" data-grid-id="grdList9" 
		...
		data-tpl-url="<c:url value='/static/gridTemplate/mem/MB175.xml'/>">
	</div>
</div>
...
function search10()
{
    var url = "<c:url value='${urlPrefix}/getCalculateStatusSAPSynthesisList${urlSuffix}'/>";
...
function calculateStatusSAPSynthesis()
{
    if (!confirm("SAP PNT 가맹점 포인트 보전 완료 처리를 하시겠습니까?"))
        return;
    ...
    var url = "<c:url value='${urlPrefix}/saveCalculateStatusSAPSynthesis${urlSuffix}'/>";
    Utilities.getAjax(url, param, true, function(data, jqXHR) {
        if (Utilities.processResult(data, jqXHR, "SAP PNT 가맹점 포인트 보전 완료 처리가 실패했습니다.")) {
            alert("SAP PNT 가맹점 포인트 보전 완료 처리가 완료되었습니다.");
        }
    });
 }
//setToday(); 
