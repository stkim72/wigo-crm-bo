	...
	<div class="mBox1">
		<div class="gTitle1">
			<h3 class="mTitle1">대분류 목록</h3>
				<div class="gRt">
				<a href="#" data-click="exportExcel" id="excelDownload" class="mBtn1 m lWhite">엑셀다운로드</a>
...
<form role="form" id="frmSearch">
			...
			<ul >
				<li class="w100per">
                     ...
                     <input type="text" id="comnCdNmLike" name="comnCdNmLike" data-enter="search" class="it" placeholder="코드명" />
	...
	<div class="mBox1 ">
		<div class="gTitle1">
			<h3 class="mTitle1">소분류</h3>
				<div class="gRt">
				<code:btnGroup name="CodeDetail" gridId="grdDetail" hasSort="true" hideExcel="true"/>
				</div>
		</div>
		<div >
...
function sortCodeDetail(){
	var list = grdDetail.getJsonRows();
	if(list.length<=1)
	{
		alert("순서변경을 위해서는 최소 2개 이상의 행이 필요합니다.");
		return;
		
	}	
	Utilities.sortList(list,"comnCdNm");
}

function changeSeq(list){
	var arr = [];
	for(var i=0;i<list.length;i++){
		list[i].comnCdOdrg = i+1;
		var dt = {
				comnCdOdrg : i+1,
				comnCd : list[i].comnCd,
				topComnCd : list[i].topComnCd,
				prntsComnCd : list[i].prntsComnCd
		};
		arr.push(dt);
	}
	var url = "<c:url value='${urlPrefix}/saveSeq${urlSuffix}'/>";
...
function exportExcel(){
 	var url = "<c:url value='${urlPrefix}/getExcelList${urlSuffix}'/>";
	var param = Utilities.formToMap("frmSearch");
	param.recordCountPerPage = 100000;
	Utilities.getAjax(url, param, true, function(data, jqXHR) {
		if (Utilities.processResult(data, jqXHR, "검색된 코드가 존재하지 않습니다.")) {
			Utilities.downloadExcel("공통코드.xlsx",data,{bgCd:"대분류코드",bgNm : "대분류명",bgYn : "대분류사용",smCd:"소분류코드",smNm : "소분류명",smYn : "소분류사용"});
		}
	});
}
</script>