...
<div class="cont">
    ...
    <div class="mBox1">
    	<div class="gTitle1">
            ...
            <div class="gRt">
                <a type="button" data-grid-id="grdList" data-click="addFile" id="btnAdd" class="mBtn1 m lWhite">
		         파일추가
		    </a>
		    <a type="button" data-grid-id="grdList" data-click="uploadFile" id="btnUpload" class="mBtn1 m lWhite">
		         upload
		    </a>
		    <a type="button" data-grid-id="grdList" data-click="downloadFile" id="btnDownload" class="mBtn1 m lWhite">
		         다운로드
		    </a>
		    <a type="button" data-grid-id="grdList" data-click="removeFile" id="btnDelList" data-action="del" class="mBtn1 m ">
		         삭제
		    </a>
            </div>
        </div>
        ...
        <div id="divGrid"  style="height: 395px" data-get-url="<c:url value='${urlPrefix}/file/getList${urlSuffix}'/>" 
                data-grid-id="grdList" data-pagenation="N" data-type="grid" data-tpl-url="<c:url value='/static/gridTemplate/file/filePopupCont.xml'/>">
    	</div>
...
<script>
    ...
    function downloadAttFile(elem,data){
    	onDownloadfile(data.fileCd, data.fileOdrg, data.fileSize);
    }
    ...
    function onGridHtmlRender(gridView, row, col, data,colValue) {
        if (data.stat != 'c') {
            ...
            //          var html = "download";
             var html = '<div data-file-cd="'+data.fileCd+'" data-file-seq="'+data.fileOdrg+'" data-type="downloadInfo"><a href="#;" id="btnDownload" class="mBtn1 m lWhite" data-click="downloadAttFile"  data-file-cd="'+data.fileCd+'"  data-file-size="'+data.fileSize+'"  data-file-odrg="'+data.fileOdrg+'" data-file-seq="'+data.fileOdrg+'" ><i class="fas fa-download"></i> 다운로드</a></div>';
            return html;
        } else {
