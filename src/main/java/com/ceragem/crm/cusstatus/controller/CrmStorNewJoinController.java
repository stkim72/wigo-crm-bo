...
import com.ceragem.crm.cusstatus.service.CrmStorNewJoinService;

import lombok.RequiredArgsConstructor;

@Controller
@RequestMapping(value = { "memdash4" })
@RequiredArgsConstructor
public class CrmStorNewJoinController {
	private final CrmStorNewJoinService service;

	@GetMapping(value = { "", "index" })
	public String init(Model model) throws Exception {
		model.addAttribute("barLabels", service.barLabels());
		model.addAttribute("newJoinMonData", service.newJoinMonData());
		model.addAttribute("lineLabels", service.lineLabels());
		model.addAttribute("newJoinYearData", service.newJoinYearData());
		return Utilities.getProperty("tiles.crm") + "cusStatus/memdash4";
	}
